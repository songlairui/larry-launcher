...
int grubCacheSeconds(HttpClientResponse response) {
...
typedef FutureOr<Map> Loader(String url, int forceCache);

Future<Map> defaultLoader(String url, int forceCache) async {
  ...
  int ttl = forceCache ?? grubCacheSeconds(response);
  Uint8List bytes = await consolidateHttpClientResponseBytes(response);
  return {'ttl': ttl, 'bytes': bytes};
}
...
class IconFileCache {
  ...
  static Future<IconFileCache> fromDefault({
    Loader loader: defaultLoader,
    String path,
    bool scan: false,
  }) async {
    if (_instance == null) {
      final Completer<IconFileCache> completer = Completer<IconFileCache>();
      if (path == null) {
        Directory dir = await getTemporaryDirectory();
        path = "${dir.path}/cache2";
      }

      final IconFileCache fileCache = new IconFileCache(
        path: path,
        useMemory: false,
        loader: loader,
      );
      ...
      if (scan) {
        fileCache.scanFolder().then((ScanResult res) {
          print("IconFileCache in scan, delete ${res.deleteCount} file.");
  ...
  static Future<IconFileCache> _instance;
  ...
  Future<Uint8List> getBytes(
    ...
    Map payload = await loader(url, forceCache);
    final Uint8List bytes = payload['bytes'];
    if (bytes.lengthInBytes == 0) throw new Exception('empty file: $url');
